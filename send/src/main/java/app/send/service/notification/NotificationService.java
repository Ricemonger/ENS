...
public class NotificationService {
    ...
    private final NotificationClient contactClient;

    public Notification findOneByPrimaryKey(String token, String notificationName) {
        List<Notification> list = contactClient.findAllByPrimaryKey(token, new NotificationNameRequest(notificationName));
    ...
    public Map<String, String> getMap(String token) {
        List<Notification> list = contactClient.findAllByUsername(token);
