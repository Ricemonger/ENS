...
import app.security.user.service.any_user.AnyUserDetails;
...
public class JwtAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (accountId != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            AnyUserDetails anyUserDetails = (AnyUserDetails) ensUserDetailsService.loadUserByUsername(accountId);
            if (jwtUtil.isTokenValid(token, anyUserDetails)) {
                log.trace("Token {} is valid, authorizing...", token);
                UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(anyUserDetails, null, anyUserDetails.getAuthorities());
